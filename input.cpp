...
void UpdatePad(void)
{
	...
	for (i = 0; i < padCount; i++)
	{
		...
		//左アナログスティック(アナログ操作)
		//ANALOG上							 
		if (dijs.lX == 0 && dijs.lY < 0)			padState[i] |= BUTTON_ANALOG_L_UP;
		//ANALOG右上
		if (dijs.lX > 0 && dijs.lY < 0)				padState[i] |= BUTTON_ANALOG_L_RIGHTUP;
		//ANALOG右
		if (dijs.lX > 0 && dijs.lY == 0)			padState[i] |= BUTTON_ANALOG_L_RIGHT;
		//ANALOG右下
		if (dijs.lX > 0 && dijs.lY > 0)				padState[i] |= BUTTON_ANALOG_L_RIGHTDOWN;
		//ANALOG下
		if (dijs.lX == 0 && dijs.lY > 0)			padState[i] |= BUTTON_ANALOG_L_DOWN;
		//ANALOG左下
		if (dijs.lX < 0 && dijs.lY > 0)				padState[i] |= BUTTON_ANALOG_L_LEFTDOWN;
		//ANALOG左
		if (dijs.lX < 0 && dijs.lY == 0)			padState[i] |= BUTTON_ANALOG_L_LEFT;
		//ANALOG左上
		if (dijs.lX < 0 && dijs.lY < 0)				padState[i] |= BUTTON_ANALOG_L_LEFTUP;

		////右アナログスティック(アナログ操作)
		////ANALOG上							 
		//if (dijs.lRx == 32895 && dijs.lRy < 32638)			padState[i] |= BUTTON_ANALOG_R_UP;
		////ANALOG右上
		//if (dijs.lRx > 32895 && dijs.lRy < 32638)				padState[i] |= BUTTON_ANALOG_R_RIGHTUP;
		////ANALOG右
		//if (dijs.lRx > 32895 && dijs.lRy == 32638)			padState[i] |= BUTTON_ANALOG_R_RIGHT;
		////ANALOG右下
		//if (dijs.lRx > 32895 && dijs.lRy > 32638)				padState[i] |= BUTTON_ANALOG_R_RIGHTDOWN;
		////ANALOG下
		//if (dijs.lRx == 32895 && dijs.lRy > 32638)			padState[i] |= BUTTON_ANALOG_R_DOWN;
		////ANALOG左下
		//if (dijs.lRx < 32895 && dijs.lRy > 32638)				padState[i] |= BUTTON_ANALOG_R_LEFTDOWN;
		////ANALOG左
		//if (dijs.lRx < 32895 && dijs.lRy == 32638)			padState[i] |= BUTTON_ANALOG_R_LEFT;
		////ANALOG左上
		//if (dijs.lRx < 32895 && dijs.lRy < 32638)				padState[i] |= BUTTON_ANALOG_R_LEFTUP;

		//十字キー(デジタル操作)
		//DIGITAL上
		if (dijs.rgdwPOV[0] == 0)				padState[i] |= BUTTON_DIGITAL_UP;
		//DIGITAL右上
		if (dijs.rgdwPOV[0] == 4500)			padState[i] |= BUTTON_DIGITAL_RIGHTUP;
		//DIGITAL右
		if (dijs.rgdwPOV[0] == 9000)			padState[i] |= BUTTON_DIGITAL_RIGHT;
		//DIGITAL右下
		if (dijs.rgdwPOV[0] == 13500)			padState[i] |= BUTTON_DIGITAL_RIGHTDOWN;
		//DIGITAL下
		if (dijs.rgdwPOV[0] == 18000)			padState[i] |= BUTTON_DIGITAL_DOWN;
		//DIGITAL左下
		if (dijs.rgdwPOV[0] == 22500)			padState[i] |= BUTTON_DIGITAL_LEFTDOWN;
		//DIGITAL左
		if (dijs.rgdwPOV[0] == 27000)			padState[i] |= BUTTON_DIGITAL_LEFT;
		//DIGITAL左上
		if (dijs.rgdwPOV[0] == 31500)			padState[i] |= BUTTON_DIGITAL_LEFTUP;
